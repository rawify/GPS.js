...
const tests = {
  ...
  '$GPRMC,081836,A,3751.65,S,14507.36,E,000.0,360.0,130998,011.3,E*62': {
    ...
    'time': new Date('1998-09-13T08:18:36.000Z'),
    'track': 360,
    'type': 'RMC',
    "navStatus": null,
    'faa': null,
  ...
  '$GPRMC,123519,A,4807.038,N,01131.000,E,022.4,084.4,230394,003.1,W*6A': {
    ...
    'speed': 41.4848,
    'status': 'active',
    'time': new Date('1994-03-23T12:35:19.000Z'),
    'track': 84.4,
    'type': 'RMC',
    "navStatus": null,
    'valid': true,
...
describe('NMEA protocol details', function () {

  it('applies the ZDA minute offset with the sign of the hour offset', function () {
    const result = GPS.Parse('$GPZDA,201530.00,04,07,2002,-05,30*4B');

    assert.equal(result.offsetMin, -330);
  });

  it('recognizes QZSS and BeiDou talker IDs', function () {
    assert.equal(GPS.Parse('$QZGSV,1,1,01,01,45,180,40*59').system, 'QZSS');
    assert.equal(GPS.Parse('$BDGSV,1,1,01,13,31,064,27*5E').system, 'BeiDou');
  });

  it('rejects checksum fields that are not exactly two hexadecimal digits', function () {
    assert.strictEqual(GPS.Parse('$GPGLL,4916.45,N,12311.12,W,225444,A,*1Dxx'), false);
    assert.strictEqual(GPS.Parse('$GPGLL,4916.45,N,12311.12,W,225444,A,*ZZ'), false);
  });

  it('does not count duplicate TXT segments twice', function () {
    const receiver = new GPS();
    const messages = [];
    receiver.on('TXT', data => messages.push(data));

    receiver.update('$GNTXT,02,01,02,a multipart message^2C this is part 1^0D^0A*34');
    receiver.update('$GNTXT,02,01,02,a multipart message^2C this is part 1^0D^0A*34');
